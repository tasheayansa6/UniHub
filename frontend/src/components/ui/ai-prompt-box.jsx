  ...
  return (
    <>
      <PromptInput value={input} onValueChange={setInput} isLoading={isLoading} onSubmit={handleSubmit}
        ...
        disabled={isLoading || isRecording} ref={ref || promptBoxRef} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}
      >
        ...
        {isRecording && (
          <VoiceRecorder isRecording={isRecording} onStartRecording={() => {}} onStopRecording={(duration) => { setIsRecording(false); onSend(`[Voice message - ${duration} seconds]`, []); }} />
        )}
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          <div className={cn("flex items-center gap-1 transition-opacity duration-300", isRecording ? "opacity-0 invisible h-0" : "opacity-100 visible")}>
            ...
            <div className="flex items-center">
              {[
                { key: "search", label: "Search", icon: Globe, color: "#1EAEDB", bg: "#1EAEDB", active: showSearch },
                { key: "think", label: "Think", icon: BrainCog, color: "#8B5CF6", bg: "#8B5CF6", active: showThink },
                { key: "canvas", label: "Canvas", icon: FolderCode, color: "#F97316", bg: "#F97316", active: showCanvas },
              ].map(({ key, label, icon: Icon, color, active }, i) => (
                <React.Fragment key={key}>
                  {i > 0 && <CustomDivider />}
                  <button type="button"
                    onClick={() => key === "canvas" ? setShowCanvas((p) => !p) : handleToggleChange(key)}
                    ...
                    style={active ? { background: `${color}26`, borderColor: color, color } : {}}
                  >
